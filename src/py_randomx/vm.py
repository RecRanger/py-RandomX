...
from py_randomx.AES import fillAes1Rx4, fillAes4Rx4, hashAes1Rx4
from py_randomx.blake import BLAKE2b
from py_randomx.common import *
from py_randomx.program import Program
...
class VM(Program):
    ...
    def initialize(self):
        ...
        self.datasetOffset = (
            self.getEntropy(13) % (DatasetExtraItems + 1)
        ) * CacheLineSize
        ...
        for _ in range(RANDOMX_PROGRAM_ITERATIONS):
            ...
            for i in range(RegisterCountFlt):
                ...
                p = unpack(
                    ...
                    pack(
                        "II",
                        self.scratchpadRead(addr, offset=4),
                        self.scratchpadRead(addr + 4, offset=4),
                    ),
                )
                self.reg.f[i]["lo"] = p[0]
                self.reg.f[i]["hi"] = p[1]
            ...
            for i in range(RegisterCountFlt):
                addr = spAddr0 + 16 * i
                self.scratchpad[addr : addr + 16] = pack(
                    "dd", self.reg.f[i]["lo"], self.reg.f[i]["hi"]
                )
